  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
      ...
      <motion.div
        ...
        className="relative w-full max-w-6xl h-full max-h-[90vh] bg-[#020617] border border-slate-900 rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden"
      >
        {/* Header */}
        <div className="p-10 md:p-14 pb-0 flex items-start justify-between">
          <div className="space-y-4">
            <h1 className="text-5xl md:text-6xl font-black tracking-tighter text-white font-display">Settings</h1>
            <p className="text-slate-500 font-medium text-lg md:text-xl max-w-xl">
              Manage curricula, backups, and the logic that shapes your study dashboard.
            </p>
          </div>
          <button 
            onClick={onClose}
        ...
        {/* Content - Bento Grid */}
        <div className="flex-1 overflow-y-auto p-10 md:p-14 grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-14">
            ...
            <section className="space-y-6">
              ...
              <div className="glass-card bg-[#0f172a]/50 p-8 border-slate-800 space-y-6 rounded-[2rem]">
                <div className="space-y-2">
                  <h4 className="text-2xl font-black text-white">Guest vs Cloud</h4>
                  <p className="text-slate-500 text-sm font-medium leading-relaxed">
                    View who is learning in this workspace and sync status.
                  </p>
                </div>
                ...
                <div className="bg-[#020617] p-6 rounded-2xl border border-slate-800 flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">Logged in as</p>
          ...
          <div className="space-y-12 text-left">
            {/* Section: Appearance */}
            <section className="space-y-6">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-500 font-display">Appearance</h3>
              <div className="glass-card bg-[#0f172a]/50 p-8 border-slate-800 space-y-6 rounded-[2rem]">
                <div className="flex items-center justify-between">
                  <div className="space-y-2">
                    <h4 className="text-2xl font-black text-white font-display">Visual Theme</h4>
                    <p className="text-slate-500 text-sm font-medium leading-relaxed">Toggle between light and dark modes in this workspace.</p>
                  </div>
                  <button 
                    onClick={() => setTheme(settings.theme === 'dark' ? 'light' : 'dark')}
                    className="p-5 bg-slate-900 border border-slate-800 rounded-2xl flex items-center justify-center gap-3 hover:bg-slate-800 transition-all text-white min-w-[140px]"
                  >
                    {settings.theme === 'dark' ? <Sun className="w-6 h-6 text-yellow-500" /> : <Moon className="w-6 h-6 text-blue-500" />}
            ...
            <div className="p-10 text-center bg-slate-900/40 rounded-[2.5rem] border border-slate-900/50 backdrop-blur-sm">
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] mb-3 font-display">Examora Stable</p>
              <p className="text-white/20 text-[10px] font-black tracking-widest">VERSION 1.5.2 • BUILT FOR PRODUCTIVITY</p>
            </div>
          </div>
        </div>
      </motion.div>
